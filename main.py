...
class DesignApp(QMainWindow):
    ...
    def init_ui(self) -> None:
        ...
        header = QLabel(
            "Выберите две Google-таблицы, папку с TXT-дизайнами и файл для обработки."
        )
    ...
    def _on_resources_loaded(
        ...
        self.status_label.setText(
            ...
            "Выберите папку, чтобы увидеть TXT-файлы дизайнов."
        )
    ...
    def on_submit(self) -> None:
        ...
        if not journal or not base or not folder or not file:
            self._show_warning("Выберите журнал, базу, папку и TXT-файл дизайна.")
            return
        ...
        self._set_busy(True, "Передача выбранных ID в обработку...")
        self._set_controls_enabled(False)
...
def run_processing(
    ...
    result = processor.run_full_processing(file_id, journal_id, base_id, services)
    return str(result)
